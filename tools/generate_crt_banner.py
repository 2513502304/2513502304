...
def get_github_name(username: str) -> str:
    try:
        req = urllib.request.Request(
            f"https://api.github.com/users/{username}",
            headers={
                "Accept": "application/vnd.github.v3+json",
                "User-Agent": "crt-banner",
            },
        )
...
github_name = get_github_name(GITHUB_USERNAME)
...
CONFIG = {
    ...
    "frames": 6,
    "frame_duration": 220,
    "portrait_source": "images/portraits/yande.re_1138000_katou_megumi_misaki_kurehito_saenai_heroine_no_sodatekata_seifuku.jpg",
}
...
def draw_bar_block(
    draw: ImageDraw.ImageDraw, x: int, y: int, width: int, height: int, fill, accent
) -> None:
    draw.rounded_rectangle((x, y, x + width, y + height), radius=2, fill=fill)
    draw.line((x + 2, y + 2, x + width - 2, y + 2), fill=accent, width=1)
    draw.line(
        (x + 2, y + height - 3, x + width - 2, y + height - 3), fill=accent, width=1
    )
...
def color_distance(
    pixel: tuple[int, int, int], ref: tuple[float, float, float]
) -> float:
...
def build_focus_mask(size: tuple[int, int]) -> Image.Image:
    ...
    draw.rounded_rectangle(
        (width * 0.18, height * 0.47, width * 0.90, height * 0.86),
...
def palette_for_luminance(
    luminance: float, edge_strength: float
...
def build_portrait_blocks(
    project_root: Path,
) -> list[tuple[int, int, tuple[int, int, int], tuple[int, int, int]]]:
...
def fit_cover(
    image: Image.Image, size: tuple[int, int], centering: tuple[float, float]
) -> Image.Image:
    return ImageOps.fit(
        image, size, method=Image.Resampling.LANCZOS, centering=centering
    )
...
def prepare_portrait_panel(project_root: Path, cfg: dict) -> Image.Image:
    ...
    contained = ImageOps.contain(composed, (664, 632), method=Image.Resampling.LANCZOS)
...
def draw_portrait(
    image: Image.Image, portrait_panel: Image.Image, frame_num: int
) -> None:
...
def draw_info_panel(image: Image.Image, cfg: dict, frame_num: int) -> None:
    ...
    prompt_y = 628
    draw_glow_text(
        image, (48, prompt_y), cfg["prompt"], font_prompt, bright, glow, blur_radius=4
    )
    if frame_num % 2 == 0:
        cursor_x = int(
            48 + ImageDraw.Draw(image).textlength(cfg["prompt"], font=font_prompt) + 8
        )
        cursor_layer = Image.new("RGBA", image.size, (0, 0, 0, 0))
        cursor_draw = ImageDraw.Draw(cursor_layer)
        cursor_draw.rectangle(
            (cursor_x, prompt_y + 2, cursor_x + 12, prompt_y + 22),
            fill=(222, 255, 215, 255),
        )
...
def add_monitor_effects(image: Image.Image, frame_num: int) -> Image.Image:
    ...
    overlay_draw.rounded_rectangle(
        (10, 10, width - 10, height - 10), radius=30, outline=(13, 22, 13), width=24
    )
    overlay_draw.rounded_rectangle(
        (20, 20, width - 20, height - 20), radius=28, outline=(6, 12, 6), width=12
    )
    overlay_draw.line((682, 36, 682, 664), fill=(18, 42, 18, 180), width=4)

    glow_layer = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    glow_draw = ImageDraw.Draw(glow_layer)
    glow_draw.rounded_rectangle(
        (28, 28, 1172, 672), radius=26, outline=(104, 255, 126, 70), width=6
    )
